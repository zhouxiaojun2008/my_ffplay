...
static void calculate_display_rect(SDL_Rect *rect, int scr_xleft, int scr_ytop, int scr_width, int scr_height, VideoPicture *vp)
{
	...
	if (width > scr_width) {
		width = scr_width;
		height = ((int)rintx(width / aspect_ratio)) & ~1;
	}
	x = (scr_width - width) / 2;
...
static void video_image_display(VideoState *is)
{
	...
	if (vp->bmp) {
		...
		//计算显示窗的位置（两种方法）
		if(is_stretch==0){  //保持纵横比，以宽度为基准，上下有黑边
			calculate_display_rect(&rect, is->xleft, is->ytop, is->width, is->height, vp);
		}else if(is_stretch==1){ //全部铺满窗口
...
#else
		pts = pts_int * av_q2d(is->video_st->time_base); //转换成秒
		//解码成功后用于显示，也是放到另一个队列中？
...
int ffmfc_play(LPVOID lpParam)
{
	...
	if (!display_disable) {
        ...
        //OutputDebugStringA("wuhan");
		fs_screen_width = myInfo->current_w;
		fs_screen_height = myInfo->current_h;
#endif
	}
